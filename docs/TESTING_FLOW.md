...
| Windows only: `prisma generate` (via `npm run db:generate-all` or `predev`) fails with `EPERM: operation not permitted` on `query_engine-windows.dll.node` | Another Node process (a dev server, a Jest/test watcher, an IDE-run script) still has the previous engine DLL loaded, so Windows won't let `prisma generate` overwrite it (`diagnose-prisma-engines.ps1`'s doc comment, lines 6-9) | **Windows/PowerShell only** — run `pwsh scripts/diagnose-prisma-engines.ps1` from the repo root; it lists every `query_engine-windows.dll.node` under `services/` and marks each `[LOCKED]`/`[ok]` (it opens each file exclusively and reports failure as locked, `diagnose-prisma-engines.ps1:31-45`). Stop the Node process holding a `[LOCKED]` file (`Get-Process node \| Select-Object Id, Path` per the script's own hint at line 61) and re-run `prisma generate`/`npm run prod:preflight`. This script will not run on macOS/Linux (`pwsh` may not even be installed) — on those platforms, find the offending process with `lsof \| grep query_engine` or simply stop all `node`/`tsx` processes and retry |
...
| Windows: `prisma generate` fails with a file-lock error mid-migration | Same `query_engine-windows.dll.node` lock issue as Section 13.2 | **Windows-only**: `pwsh scripts/diagnose-prisma-engines.ps1` to find and stop the process holding the lock |
