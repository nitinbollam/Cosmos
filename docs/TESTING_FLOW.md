...
| /api/v1/health/db | GET | ADMIN_ROLES | Checks that all 14 `*_DATABASE_URL` env vars are configured, plus a real `SELECT 1` connectivity ping against `authDb` only — the other 13 schemas are checked for a non-empty URL string, not live connectivity (`checkDatabaseConnections`, `db-health.ts:9-43`, `native-router.ts:86-89`; see Section 12.1) |
...
**What to verify:** pick a Tier 1 handler that imports two `*Db` clients (e.g. `order-orchestration.ts`'s checkout flow) and confirm in a debugger/log that it issues two independent Prisma calls rather than one; hit `GET /api/v1/health/db` (8.2.16) and confirm the response reports one entry per `*_DATABASE_URL` env key plus an `auth_connectivity` entry (`checkDatabaseConnections`, `db-health.ts:9-43`, `native-router.ts:86-89`) — but note that only `auth_connectivity` is a real `SELECT 1` probe; the other 13 entries only confirm their env var is a non-empty string, so a genuinely down non-auth schema will still report `ok: true` here rather than showing up as a failing entry (see Section 12.1).
...
This section documents failure modes that follow directly from how the codebase is actually built — the 14-schema Prisma layout, the manual (non-middleware) tenant-scoping convention, and the shared error-to-HTTP-status mapping — rather than generic advice. Each item cites the exact source that produces the behavior.

### 12.1 Multi-Schema Prisma Pitfalls

`apps/web/lib/server/db.ts` exports 14 independent `PrismaClient` singletons (`authDb`, `tenantDb`, `inventoryDb`, `orderDb`, `crmDb`, `storefrontDb`, `wmsDb`, `dispatchDb`, `purchasingDb`, `paymentDb`, `complianceDb`, `notificationDb`, `ledgerDb`, `analyticsDb`; `db.ts:33-46`), each generated from its own `apps/web/prisma/<schema>/schema.prisma` into its own `@/generated/prisma-<schema>` package (`db.ts:1-14`). There is no cross-schema client and no runtime schema switch (Section 9.2).

| Failure | Why it happens | What you'll see |
|---|---|---|
| "Property does not exist" TypeScript error when a handler tries to query a model that lives in a different schema | Each generated client's type only exposes the models declared in *that* schema's `.prisma` file — e.g. `orderDb.sKU` doesn't compile because `SKU` is declared in `apps/web/prisma/inventory/schema.prisma`, not `apps/web/prisma/order/schema.prisma` | A `tsc`/`npm run typecheck` failure at build time, **not** a runtime error — this class of "wrong schema client" mistake is caught before the app ever starts, per-model, by the TypeScript compiler |
| Cross-schema foreign keys are unenforced strings | A record in one schema that references a row in another schema (e.g. `payment/schema.prisma:14`'s `VendorBillPayment.orderId String` — no `@relation`, because `Order` lives in the `order` schema, not `payment`) is a plain indexed string column with **no DB-level foreign-key constraint and no `onDelete` cascade**, unlike same-schema references (e.g. `order/schema.prisma:57`'s `OrderLineItem.order Order @relation(fields: [orderId], references: [id], onDelete: Cascade)`, which Postgres/SQLite does enforce) | Deleting or corrupting the "parent" row in its own schema does not fail, warn, or cascade — the child row in the other schema is silently orphaned. There is no code today that deletes `Order` rows, so this is a latent risk rather than an observed bug; watch for it if a future "purge old orders" feature is added without an explicit cross-schema cleanup step |
| A handler that needs two schemas issues two queries, and the second can fail independently of the first | Prisma cannot join across separate clients, so any multi-schema operation (e.g. `order-orchestration.ts`'s checkout, which imports both `orderDb` and `paymentDb`) is not one atomic DB transaction — it's two separate Prisma calls, each with its own connection/transaction scope | If the second call throws after the first committed, the two schemas can end up inconsistent (e.g. an `Order` row created in `order` without a matching successful payment record in `payment`); there is no distributed-transaction/saga rollback across schema clients in this codebase, so error-handling in the calling module (not the DB) is what has to compensate |
| `GET /api/v1/health/db` reporting `allOk: true` doesn't mean every schema's database is actually reachable | `checkDatabaseConnections()` (`db-health.ts:9-43`) pushes one `{ key, ok }` entry per `WEB_DATABASE_ENV_KEYS` entry (`env.ts:22-37`) that only checks whether the env var **string is non-empty** (`db-health.ts:17-24`) — it does **not** open a connection for 13 of the 14 schemas. Only `authDb` gets a real `SELECT 1` query (`db-health.ts:26-36`, the `auth_connectivity` check) | A misconfigured or unreachable `INVENTORY_DATABASE_URL` (wrong host, DB down) will still show `ok: true` in the response as long as the string itself is non-empty — the endpoint only proves the auth schema is live, not the other 13 |

### 12.2 Tenant/Customer Scoping Mistakes

Every tenant-scoped model uses a globally-unique `cuid()` primary key (e.g. `apps/web/prisma/inventory/schema.prisma:56`'s `SKU.id String @id @default(cuid())`), not a tenant-namespaced key. There is no Prisma middleware, `$extends`, or row-level-security layer that automatically injects a `tenantId` filter (confirmed: no `$use(`/`$extends(` call anywhere under `apps/web/lib/server`) — every function threads `tenantId` through explicitly and includes it in its own `where` clause by convention. The codebase today is consistent about this: e.g. `ap-bills.ts:73`'s `where: { id: billId, tenantId }` and `users.ts:27`/`users.ts:52`'s `authDb.user.findFirst({ where: { id, tenantId } })` before any mutation.

**The failure mode this convention makes possible:** because `id` alone is already a valid, uniquely-matching Prisma filter (the cuid is globally unique across tenants), a handler that does `db.model.findUnique({ where: { id } })` instead of `findFirst({ where: { id, tenantId } })` **compiles and runs successfully** — it just returns (or updates/deletes) a row regardless of which tenant owns it, if that id ever reaches the handler from the wrong tenant's session. Nothing at the type-checker or ORM level catches this; only code review or an explicit integration test (start two tenants, try to fetch tenant A's record with tenant B's session and confirm `404`, not the record) would.

Buyer-portal customer-scoping follows the same manual pattern: `orders.ts` and `quotes.ts` force-substitute the session's resolved `buyerCustomerId` for whatever `customerId`/`customerRef` the client body sent (`orders.ts:47-49`'s check throwing `ApiError(403, 'Cannot place orders for another customer')` if the two disagree, `quotes.ts:38-40`'s equivalent for quotes) — the same "explicit substitution in every handler, not enforced centrally" shape as tenant scoping, and the same review-only safety net if a future buyer-facing endpoint is added without repeating the substitution.

### 12.3 Error-to-HTTP-Status Surprises

Every native-router response funnels through one catch-all (`native-router.ts:118-120`) calling `toJsonError` (`session.ts:101-111`, already documented for Tier 1 in Section 8's line 1064):

- An `ApiError` instance returns `{ message }` at its own declared status (this is how the specific 400/403/404/409 messages throughout Sections 5/8/10 are produced).
- A raw Prisma `PrismaClientKnownRequestError`/error-like object with `code === 'P2002'` (unique-constraint violation) becomes a generic `{ message: 'Conflict' }` at 409 — **unless** the throwing module already caught it and re-threw a specific `ApiError` first, which several modules do explicitly: `ledger.ts:134-136` ("Account code already exists"), `fixed-assets.ts:105-107` ("Asset code already exists"), `inventory.ts:420`/`inventory.ts:470` ("SKU code already exists for this tenant"), `inventory.ts:530` ("Warehouse code already exists"). Any *other* model's unique-constraint violation that isn't caught this way falls through to the generic `{ message: 'Conflict' }` instead of a field-specific message.
- Anything else (a plain `Error`) is classified by **substring-matching its message text**: contains `"not found"` → 404, contains `"required"` → 400, otherwise → 500 (`session.ts:108-109`). This means the HTTP status of an uncaught error depends on incidental wording, not on error type — a thrown `Error('Warehouse is required to receive PO lines')` gets mapped to 400 by luck of containing "required", while a thrown `Error('Cannot receive against a closed PO')` (no matching substring) becomes a 500 even though it's really a 400-class validation failure. When debugging an unexpected 500, check whether the underlying module threw a plain `Error` instead of an `ApiError` before assuming it's a real server fault.

### 12.4 Auth/Session Failure Modes

- **Login rate limiting.** `POST /api/v1/auth/login` calls `rateLimit` twice (`api-router.ts:83-84`) — 20 attempts per IP and 10 attempts per email address, both within a 5-minute window (`auth-security.ts:35-46`) — before calling `loginUser`. Exceeding either throws `ApiError(429, 'Too many attempts. Try again in <n>s')`. A test suite or script that retries failed logins in a tight loop (e.g. re-testing a wrong-password case repeatedly) will start seeing 429s instead of the expected 401 after the 10th/20th attempt in the window.
- **Stale role/active-state for up to 60 seconds.** `getSession` caches each user's `isActive`/`role` from the auth DB for `USER_STATE_TTL_MS = 60_000` ms (`session.ts:15-16, 18-35`) so that every authenticated request doesn't re-query the DB. `users.ts`'s `deactivateUser` (`users.ts:26-34`) and `updateUser` (`users.ts:47-73`) both call `invalidateUserSessionCache(id)` immediately after writing, so admin-console deactivations/role-changes take effect on the very next request. But any state change that bypasses those two functions — a raw DB edit, a seed script re-run, a direct Prisma call from a one-off script — leaves the cached role/active flag stale for up to a minute, which can look like "I deactivated the user but their session still works."
- **`JWT_SECRET` missing at boot.** `jwtSecret()` (`env.ts:1-5`) throws a plain `Error('JWT_SECRET is not configured')` the first time anything tries to sign or verify a token, rather than failing at process start — so the app can appear to boot cleanly and only fail the first time someone logs in or hits any authenticated route.
- **Feature-gated 403s that look like bugs.** `assertFeature(tenantId, feature)` (`feature-flags.ts:47-51`) throws `ApiError(403, 'Feature "<feature>" is not enabled on your plan')` for any tenant whose plan doesn't include it — e.g. Celestial AI (Section 5.13/9.5) gates on `'celestial'`. A blank/error response from a feature that "should just work" is often this check, not an auth bug — confirm the tenant's plan/feature flags before treating it as a defect.

### 12.5 Embedded SQLite Concurrency

Each of the 14 schemas is its own single-writer SQLite file under `apps/web/.data/` in local dev (`scripts/db-urls.mjs`'s `DB_BY_SCHEMA` map, `scripts/setup-sqlite.mjs`). SQLite serializes writers per file; running two processes against the same `.data` directory at once (e.g. a second `npm run dev` left running in another terminal, or a test run against the same data dir as a live dev server) can produce a `SQLITE_BUSY: database is locked` error from whichever schema client loses the race. `db.ts:48-63` caches all 14 clients on `globalThis` outside of `NODE_ENV=production` specifically to prevent Next.js's dev-mode hot-reload from creating a fresh set of 14 `PrismaClient` instances (and connections) on every file save — if that guard were ever removed, repeated hot reloads would be a second, self-inflicted source of the same lock contention.

## 13. Troubleshooting Guide

Each row is symptom → likely cause → the exact command to run. Commands are copy-pasteable from the repo root unless noted otherwise.

### 13.1 Startup Issues

| Symptom | Likely cause | Fix |
|---|---|---|
| `npm run dev` exits immediately with "Missing root .env" | `scripts/prepare-dev-env.mjs` (step 1 of the `dev` script, Section 3.1) found neither `.env` nor `.env.example` at the repo root | `cp .env.example .env` from the repo root, then re-run `npm run dev` (the script auto-copies `.env.example` → `.env` on its own if only `.env` is missing, but exits with an error if `.env.example` is also missing — Section 3.3's tip) |
| `npm run dev` fails during the Prisma-generate step with no further explanation | One of the 14 sequential `prisma generate --schema prisma/<x>/schema.prisma` calls in `apps/web/package.json`'s `db:generate` script failed and the `&&` chain stopped there | Re-run `npm run db:generate-all` (`scripts/generate-all-prisma.mjs`) directly to see which schema's `prisma generate` failed and read that schema's own error output |
| Server starts but every request 500s with a Prisma/database error | The `.env`/`apps/web/.env.local` doesn't have a valid `*_DATABASE_URL` for one or more of the 14 schemas, or `npm run db:setup`/`db:setup:postgres` was never run | Re-run `npm run predev` (`prepare-dev-env.mjs && generate-all-prisma.mjs`) to regenerate `apps/web/.env.local`, and confirm `npm run db:setup` (SQLite) has created `apps/web/.data/*.db` or `db:setup:postgres` succeeded |
| Server starts but `startBackgroundJobs()`'s immediate sweep throws at boot | `background-jobs.ts`'s `releaseExpiredReservations()` (via `inventory.ts`) failed, most commonly because the `inventoryDb` schema isn't migrated/generated yet | Confirm `npm run db:generate-all` and `npm run db:migrate` (Postgres) or `npm run db:setup` (SQLite) both completed before starting the app |

### 13.2 Build Failures

| Symptom | Likely cause | Fix |
|---|---|---|
| `npm run prod:preflight` fails at the lint/test/build step | `scripts/production-preflight.mjs` runs `npm run lint`, then `npm run test`, then `npm run build` in sequence and **exits at the first non-zero status** (`production-preflight.mjs:12-20`) — this is the exact gate `.github/workflows/ci.yml`'s "Preflight (lint · test · build)" step runs before the Docker image job, with `DATABASE_URL`/`JWT_SECRET`/`JWT_REFRESH_SECRET` set against a real Postgres service container | Run `npm run lint`, `npm run test`, `npm run build` individually to isolate which of the three stages is failing — the preflight script itself doesn't say which one beyond the inherited stdio output |
| Windows only: `prisma generate` (via `npm run db:generate-all` or `predev`) fails with `EPERM: operation not permitted` on `query_engine-windows.dll.node` | Another Node process (a dev server, a Jest/test watcher, an IDE-run script) still has the previous engine DLL loaded, so Windows won't let `prisma generate` overwrite it (`diagnose-prisma-engines.ps1`'s doc comment, lines 6-9) | **`scripts/diagnose-prisma-engines.ps1` is stale — do not rely on it as-is.** It searches `Join-Path $root 'services'` for `query_engine-windows.dll.node` and errors out immediately if `services/` doesn't exist (`diagnose-prisma-engines.ps1:17-20`). The `services/*` microservices tree it was written against was deleted in a later commit that touched this same script only to fix an unrelated `pnpm`→`npm` string (commit `db8f3d204`) — `services/` is confirmed absent from the repo today. The engine binaries it's looking for now actually live at `apps/web/generated/prisma-<schema>/` (one folder per schema, e.g. `apps/web/generated/prisma-auth/`; on Windows the file there would be `query_engine-windows.dll.node`, the same name the script greps for — on macOS/Linux it's `libquery_engine-<platform>.dylib.node`/`.so.node` instead). Until the script is updated to point at `apps/web/generated`, diagnose a Windows engine-lock manually: check `apps/web/generated/prisma-<schema>/query_engine-windows.dll.node` for each schema, and use `Get-Process node \| Select-Object Id, Path` to find and stop the Node process holding it open, then re-run `prisma generate`/`npm run prod:preflight` |
| Docker image build (`apps/web/Dockerfile`) fails or produces a client pointed at the wrong gateway URL | Missing/wrong `--build-arg` values — the Dockerfile declares `VITE_GATEWAY_URL`, `VITE_STRIPE_PUBLISHABLE_KEY`, `VITE_WEB_ADMIN_ORIGIN` as build args baked into the static bundle at build time (`Dockerfile:27-30`), not runtime env vars | Pass all three `--build-arg`s explicitly (see `.github/workflows/ci.yml`'s docker job for the CI values) — a missing `VITE_GATEWAY_URL` build arg silently falls back to `/api/v1` (usually fine), but a missing `VITE_WEB_ADMIN_ORIGIN` can break cross-origin admin-console redirects in that image |
| TypeScript build fails with "Property '<model>' does not exist on type 'PrismaClient'" | A handler imported the wrong schema client for the model it's querying (Section 12.1) — e.g. importing `orderDb` and calling a method that only exists on `inventoryDb` | Check which `apps/web/prisma/<schema>/schema.prisma` actually declares the model, and import the matching named export from `apps/web/lib/server/db.ts` (`db.ts:33-46`) instead |

### 13.3 API Failures

| Symptom | Likely cause | Fix |
|---|---|---|
| A validation-looking error comes back as HTTP 500 instead of 400/404 | The handler threw a plain `Error(...)` whose message doesn't happen to contain `"not found"` or `"required"` — `toJsonError`'s substring fallback (`session.ts:108-109`, Section 12.3) defaults everything else to 500 | Read the response `message` body (it's preserved verbatim even at 500) to see the real cause; if this is your own code, throw `ApiError(400/404, message)` instead of a plain `Error` so the status is explicit rather than guessed from text |
| A create/update returns 409 `{"message":"Conflict"}` with no field-specific detail | A `P2002` unique-constraint violation on a model that isn't one of the ones with a dedicated catch (Section 12.3 lists `ledger.ts`, `fixed-assets.ts`, `inventory.ts`'s SKU/warehouse code checks as the modules with specific messages) — everything else falls through to `toJsonError`'s generic `{ message: 'Conflict' }` (`session.ts:105-107`) | Cross-reference the request body against that model's `@@unique`/`@unique` fields in its `apps/web/prisma/<schema>/schema.prisma` to identify which field collided |
| `GET /api/v1/health/db` returns `allOk: true` but a specific schema's queries are actually failing | The health check only verifies non-auth schema URLs are non-empty strings, not live connectivity (Section 12.1's last row, `db-health.ts:9-43`) | Don't rely on `health/db` to catch a down/misconfigured non-auth database — query that schema directly (e.g. `curl` an endpoint backed by it) or add a temporary `$queryRaw\`SELECT 1\`` check for that specific `*Db` client |
| Two-schema operation (e.g. checkout) leaves inconsistent state after a mid-flight error | No cross-schema transaction exists — each `*Db` client call commits independently (Section 12.1) | Check the calling module's own compensation logic (e.g. `order-orchestration.ts`'s `cancelOrderWithCompensation`, referenced in Section 10.4) rather than expecting the DB layer to have rolled anything back automatically |

### 13.4 Auth Problems

| Symptom | Likely cause | Fix |
|---|---|---|
| Repeated login attempts in a test script start returning 429 instead of 401 | `rateLimit` in `api-router.ts:83-84` caps login attempts at 20/IP and 10/email per 5-minute window (`auth-security.ts:35-46`) | Space out or reduce the number of login attempts in the test/script, or restart the server process to clear the in-memory rate-limit map (there is no persistent store, so a restart resets it) |
| Deactivating/changing a user's role in the admin console doesn't seem to take effect immediately for that user's already-open session | Session role/active state is cached for up to 60 seconds (`session.ts:15-16`, Section 12.4) | Wait up to 60 seconds, or if this happened via a path other than `PATCH`-style admin user update/deactivate (which already calls `invalidateUserSessionCache`, `users.ts:34,73`), confirm the mutation actually went through `users.ts`'s functions and not a raw DB write |
| Every authenticated request suddenly 500s with "JWT_SECRET is not configured" | The `JWT_SECRET` env var is unset/empty in the running process's environment — `jwtSecret()` (`env.ts:1-5`) throws only when a token is actually signed/verified, so the process can boot fine and fail only on first login | Set `JWT_SECRET` (and `JWT_REFRESH_SECRET`) in `.env`/the environment before starting the process (Section 2.4); there is no default — this is one of the two variables `.env.example` requires you to replace |
| A tenant on a lower plan gets 403 `Feature "X" is not enabled on your plan` on a screen that should work | `assertFeature` (`feature-flags.ts:47-51`) is gating that feature for the tenant | Confirm the tenant's feature flags/plan via Settings → Integrations or the tenant record directly, rather than treating it as a session/auth bug |
| A buyer-portal session gets 403 `Cannot place orders for another customer` / `Cannot create quotes for another customer` | The request body's `customerId`/`customerRef` doesn't match the session's resolved `buyerCustomerId` (`orders.ts:47-49`, `quotes.ts:38-40`, Section 12.2) | The client should not send a `customerId` for portal-buyer requests at all (or must send exactly the buyer's own); check `buyer-context.ts`'s customer resolution if the mismatch is unexpected (e.g. the buyer's email doesn't match any CRM `Customer` record) |

### 13.5 Database Issues

| Symptom | Likely cause | Fix |
|---|---|---|
| `SQLITE_BUSY: database is locked` on some schema's queries | Two processes (e.g. two `npm run dev` instances, or a test run and a dev server) are writing to the same single-writer SQLite file under `apps/web/.data/` at once (Section 12.5) | Stop the extra process; only run one `npm run dev` (or one test run) against a given `apps/web/.data` directory at a time |
| Postgres mode: connections refused or `ECONNREFUSED` | `PLEROS_DB_PROVIDER=postgres` is set but no Postgres instance is reachable at the configured `DATABASE_URL`/`*_DATABASE_URL`s | Start the bundled Compose stack (`npm run infra:docker:up`, i.e. `docker compose -f docker-compose.yml up -d` — Section 2.1/2.5) or point at a real Postgres host, then re-run `npm run db:setup:postgres` |
| `prisma generate`/`db:migrate` fails after switching `PLEROS_DB_PROVIDER` | The `provider = "sqlite" \| "postgresql"` line inside each of the 14 `apps/web/prisma/*/schema.prisma` files wasn't rewritten to match — this only happens automatically inside `db:setup`/`db:setup:postgres` | Run `npm run db:provider` (`scripts/apply-prisma-provider.mjs`) explicitly to resync all 14 schema files with the current `PLEROS_DB_PROVIDER`, then re-run `npm run db:generate-all` |
| `GET /api/v1/health/db` (as an `ADMIN_ROLES` session) reports one schema's URL check as `false`/`"missing URL"` | That schema's `*_DATABASE_URL` env var is empty in the running process (`db-health.ts:17-24` checks `Boolean(url?.trim())` per `WEB_DATABASE_ENV_KEYS`, `env.ts:22-37`) | Regenerate `apps/web/.env.local` via `npm run predev` (`prepare-dev-env.mjs`), which fills in every `<SCHEMA>_DATABASE_URL` from the root `.env` or a computed SQLite default (`prepare-dev-env.mjs:59-64`) |
| Windows: `prisma generate` fails with a file-lock error mid-migration | Same `query_engine-windows.dll.node` lock issue as Section 13.2 | **Windows-only, and `scripts/diagnose-prisma-engines.ps1` is stale** (it still scans a deleted `services/` directory — see Section 13.2): manually check `apps/web/generated/prisma-<schema>/query_engine-windows.dll.node` for each of the 14 schemas and stop the Node process holding it open (`Get-Process node \| Select-Object Id, Path`) before retrying |

### 13.6 Environment Problems

| Symptom | Likely cause | Fix |
|---|---|---|
| App boots but every login/token operation fails | `JWT_SECRET`/`JWT_REFRESH_SECRET` still at their `.env.example` placeholder values, or unset | Set both to real random values in `.env` before starting (Section 2.2/2.4); `jwtRefreshSecret()` (`env.ts:7-9`) falls back to `jwtSecret()` if unset, so a missing refresh secret alone won't surface as its own error — only a missing `JWT_SECRET` will |
| Stripe/SendGrid/Twilio-backed features silently no-op instead of erroring | Those integrations are optional by design locally (Section 2.7/9.3) — `deliverNotification` falls back to a console/webhook no-op when their env vars are unset | This is expected in local dev; only treat it as a problem if you specifically need to exercise the real provider — set the corresponding sandbox/test credentials from Section 9.3's checklist |
| `apps/web/.env.local` / `apps/client/.env` have stale or wrong values after editing the root `.env` | These two files are **generated**, not hand-edited — `prepare-dev-env.mjs` regenerates both from the root `.env` every time `npm run dev`/`predev` runs (`prepare-dev-env.mjs:94,103`) | Edit the root `.env`, then re-run `npm run predev` (or just `npm run dev`, which calls it first) rather than editing the generated files directly — direct edits to `apps/web/.env.local` are overwritten on the next `dev`/`predev` run |
| CI's `npm run prod:preflight` step passes locally in SQLite mode but fails in CI | CI runs preflight against a real Postgres service container with `DATABASE_URL`/`JWT_SECRET`/`JWT_REFRESH_SECRET` env vars set (`.github/workflows/ci.yml`'s "Preflight" step) — a code path that behaves differently under `PLEROS_DB_PROVIDER=postgres` than under the local SQLite default can pass locally and fail in CI | Reproduce locally with `PLEROS_DB_PROVIDER=postgres npm run db:setup:postgres && npm run db:generate-all && npm run db:migrate && npm run prod:preflight` (Section 2.5) before assuming it's a CI-only flake |
| A new schema/env var was added but `GET /api/v1/health/db` doesn't report it | `WEB_DATABASE_ENV_KEYS` (`env.ts:22-37`) is a manually-maintained list, not derived at runtime from the filesystem — it has to be updated by hand alongside a new `apps/web/prisma/<schema>/schema.prisma` and its `db.ts` client export | Add the new `<SCHEMA>_DATABASE_URL` key to `WEB_DATABASE_ENV_KEYS` in `env.ts` when adding a 15th schema |
...
Each stage gates on a different, already-documented part of this file. None of these are separate tooling — they're the same checklists and commands from Sections 5–11, run in this order before a release is called done.

- **Build.** Run `npm run lint`, `npm run test`, and `npm run build` — the three root scripts (`package.json`'s `scripts`, fanned out per-workspace by `scripts/workspace-run.mjs`) and, not coincidentally, the exact three steps `scripts/production-preflight.mjs` chains in sequence for `npm run prod:preflight` (Section 13.2), which is also what `.github/workflows/ci.yml`'s "Preflight (lint · test · build)" step runs in CI against a real Postgres service container. `npm run typecheck` (`tsc --noEmit` per workspace) is a fourth root script that is **not** part of `prod:preflight` itself — run it explicitly as part of Build, since a type error can slip past lint/test/build if `build`'s own `tsc` step for a given workspace doesn't happen to touch the broken file. Any non-zero exit here stops the release before Smoke.
- **Smoke → Section 11 (Smoke Testing Checklist).** The few-minutes pass confirming the deploy isn't dead on arrival: process boots, no Prisma connection errors across the 14 schema clients, background jobs' immediate sweep doesn't throw, and one login + one page load per surface (`/admin`, buyer portal, mobile PWA). If any Section 11 item fails, stop and treat the deploy as broken — don't proceed to Functional.
- **Functional → Section 5 (Module-by-Module Testing).** Work through each module's `<details><summary>Test Checklist</summary>` block (Functional/UI validation/Form validation/Navigation/Permissions/Error handling/Edge cases/Empty state/Loading state/Success/Failure scenario items) for whichever modules the release actually touched. Section 5's per-module checklists are the acceptance criteria for "does this module still work in isolation."
- **Integration → Section 9 (Integration Testing).** Confirm the seams still hold: frontend↔API (Section 9.1, the shared `@pleros/web-gateway-client`), API↔database (Section 9.2, the 14 independent Prisma clients), API↔third-party services, the event bus/background jobs, and the Celestial AI pipeline. This is where a change that passes every individual module's Functional checklist but breaks a handoff between two modules gets caught.
- **Regression → Section 10 (Regression Testing Checklist).** Run the five cross-module journeys end to end — Order-to-Cash (10.1), Procure-to-Pay (10.2), WMS Pick/Pack/Ship (10.3), POS Sale (10.4), Fixed-Asset Lifecycle (10.5) — always before a release per Section 10's own instruction, and mandatorily whenever the release touches more than one module in a given chain.
...
### 16.1 Recommended Execution Order

Follow Section 14's stage order rather than jumping straight to Regression: **Smoke (Section 11) before Functional (Section 5) before Integration (Section 9) before Regression (Section 10)**. Smoke is deliberately cheap (a few minutes) and catches a dead-on-arrival deploy before you spend time working through Section 5's per-module checklists; Functional in turn catches a broken module before you spend time on Section 9's cross-seam checks or Section 10's multi-module journeys, which take the longest and are the most expensive to debug when the root cause turns out to be a single module that never worked in isolation. Running Regression first and working backwards on failure wastes time re-deriving which single module actually broke.

### 16.2 Efficient Debugging

- **Inspect one schema at a time with Prisma Studio.** `prisma` is a workspace devDependency (hoisted to the repo-root `node_modules/.bin/prisma`), so `npx prisma studio --schema apps/web/prisma/<schema>/schema.prisma` is a real, usable command for browsing that schema's tables directly — no separate installation needed. Because the app has 14 independent Prisma clients with no cross-schema client (Section 12.1), Studio only ever shows you one schema's tables per invocation; point it at `apps/web/prisma/order/schema.prisma`, `apps/web/prisma/inventory/schema.prisma`, etc. individually rather than expecting one Studio session to show the whole picture. This is the fastest way to confirm what a handler actually wrote (or didn't write) without adding temporary `console.log`s.
- **Read `node --test` output for what it actually tells you.** `npm run test` (`scripts/workspace-run.mjs test`) fans out to each workspace's own `test` script; for `apps/web` that's `node --import ./server/register-paths.mjs --import tsx --test lib/server/**/*.test.ts` (Section 1.4). The default `node --test` reporter prints a pass/fail line per `test(...)` block with the failing assertion's expected/actual diff inline — to iterate faster on a single failing file instead of the whole `lib/server/**` glob, `cd apps/web` and re-run the same command with the specific file's path in place of the glob.
- **Cross-reference an unexpected HTTP status against Section 12.3** before assuming a bug — the substring-matching fallback in `toJsonError` (`session.ts:108-109`) means a plain `Error('...')` without "not found"/"required" in its message becomes a 500 even when it's logically a 400.

### 16.3 Writing Reproducible Bug Reports

A report that can be replayed by someone else needs, at minimum:
- **Role** — which seeded `Role` value the session held (`SUPER_ADMIN`, `TENANT_ADMIN`, `MANAGER`, `WAREHOUSE_STAFF`, `SALES_REP`, `DRIVER`, `ACCOUNTANT`, `VIEWER`, or `STAFF` — Section 5's intro), since most failures in this codebase are role-gate or role-scoping issues (Section 12.2/12.4).
- **Tenant** — the `tenantId` involved. Because tenant scoping is manual per handler rather than middleware-enforced (Section 12.2), a bug that looks like a generic failure can actually be tenant-specific (e.g. missing chart-of-accounts data, a feature flag off for that tenant's plan per Section 12.4's `assertFeature`).
- **The exact request** — method, path, and full JSON body (or the exact UI action that produced it), matched against the documented shape in Section 8 for that endpoint. "It failed when I submitted the form" isn't reproducible; "`POST /purchase-orders/:id/receive` with `{ lines: [{ id, qtyReceived: 5 }] }`" is.
- **The screen/URL** — which of the three separate frontends the bug occurred on (`/admin/...`, the buyer portal at `/...`, or the mobile PWA at `/m/...`), since each has its own gateway-client login path (Section 9.1) and a bug on one doesn't imply the same bug exists on another.

### 16.4 Avoiding Flaky Tests Under `node --test`

The repo has 34 `*.test.ts` files today (27 under `apps/web/lib/server` including its `celestial/` subfolder, 4 under `apps/client/src/lib`, 2 under `packages/analytics-engine/src`, 1 under `packages/web-gateway-client/src`). Most are pure unit tests with no database access at all (e.g. `apps/web/lib/server/tier2.test.ts` tests `computeSalesTax`/`orderIdFromStopAddress`/`netTermsExposure` as plain functions; `po-receiving.test.ts` tests `assertPoOpenForReceiving` the same way) — nothing to isolate there. The handful that do exercise a live Prisma client (confirmed by reading `apps/web/lib/server/fixed-assets.test.ts`, `compliance-recall.test.ts`, and `dispatch-optimize.test.ts`) all follow the same pattern: **each individual `test(...)` block generates its own `randomUUID()`-based `tenantId` (and related entity ids like `skuId`/`warehouseId`/`customerId`) inline**, rather than sharing a fixed tenant across tests or across files. For example, `fixed-assets.test.ts:40` does `const tenantId = \`tenant-${randomUUID()}\`` inside the test itself. This is neither a fresh-SQLite-file-per-test setup nor transaction-rollback-per-test — it's tests running against the same shared dev database (SQLite file or Postgres instance, per Section 2.5) but never colliding on unique constraints or reading each other's rows, because every test's data lives under its own random tenant/entity ids.

When adding a new DB-touching test: generate your own `randomUUID()`-based tenant and entity ids rather than hardcoding a tenant id or reusing one from another test file, and create whatever reference data the code under test needs (e.g. `createChartAccount` before testing GL posting, as `fixed-assets.test.ts` does) rather than assuming seed data exists. Don't rely on test execution order or on a previous test's data being present — nothing enforces file or test ordering here.
