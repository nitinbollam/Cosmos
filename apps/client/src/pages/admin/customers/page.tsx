...
export default function CustomersPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-pleros-white">Customers</h1>
      ...
      <Card>
        ...
        <div className="mt-4 flex flex-col sm:flex-row gap-2">
          <input
            className="flex-1 bg-pleros-surface-2 border border-pleros-border rounded-md px-3 h-10 text-sm text-pleros-white"
      ...
      <Card>
        ...
        {isLoading ? (
          ...
          <ul className="mt-4 divide-y divide-pleros-border">
            {(data ?? []).map((c) => (
              <li key={c.id} className="py-3 flex justify-between gap-4">
                <div>
                  <Link
                    to={adminPath(`/crm/customers/${c.id}`)}
                    className="text-pleros-white font-medium hover:text-pleros-primary"
                  >
                    {c.name}
                  </Link>
                  <div className="text-xs text-pleros-muted font-mono">{c.id.slice(-14)}…</div>
                </div>
                <div className="text-right text-sm text-pleros-muted">
                  {c.email ?? '—'}
                  {c.phone ? <div>{c.phone}</div> : null}
                </div>
              </li>
            ))}
            {!data?.length && <li className="text-pleros-muted text-sm py-4">No customers yet.</li>}
          </ul>
        )}
      </Card>
    </div>
