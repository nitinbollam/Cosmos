...
export default function JournalDetailPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <Link to="/admin/finance" className="text-sm text-cosmos-muted hover:text-cosmos-white">
        ← Finance
      </Link>
      ...
      {q.isLoading ? (
        ...
        <>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-cosmos-white">Journal entry</h1>
              <p className="font-mono text-xs text-cosmos-muted mt-1">{j.id}</p>
              <div className="mt-2">
                <StatusBadge status={j.isPosted ? 'POSTED' : 'DRAFT'} />
              </div>
              <p className="text-cosmos-muted text-sm mt-3 max-w-2xl">{j.description}</p>
            </div>
            {!j.isPosted && (
              <button
                type="button"
                  ...
                  {j.lines.map((l) => (
                    <tr key={l.id} className="border-b border-cosmos-border/60">
                      <td className="py-2 pr-4">
                        <span className="font-mono text-cosmos-text">{l.account?.code ?? l.accountId}</span>
                        <span className="text-cosmos-muted"> — {l.account?.name ?? '—'}</span>
                      </td>
                      <td className="py-2 pr-4 text-cosmos-muted max-w-[200px] truncate">{l.memo ?? '—'}</td>
                      <td className="py-2 pr-4 font-mono text-cosmos-text">
                        {Number(l.debit) > 0 ? Number(l.debit).toFixed(2) : '—'}
                      </td>
                      <td className="py-2 font-mono text-cosmos-text">
          ...
          <Card>
            ...
            <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-cosmos-muted">Last updated</dt>
                <dd className="text-cosmos-text">{new Date(j.postedAt).toLocaleString()}</dd>
              </div>
              <div>
                <dt className="text-cosmos-muted">Fiscal period closed flag</dt>
                <dd className="text-cosmos-text">{j.fiscalPeriodClosed ? 'Yes' : 'No'}</dd>
              </div>
            </dl>
          </Card>
        </>
