...
export function getNotificationProviderStatus(): NotificationProviderStatus {
  ...
  return {
    ...
    setupNote:
      'Configure SENDGRID_API_KEY (+ SENDGRID_FROM_EMAIL) for email, TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM_NUMBER for SMS, or NOTIFICATION_WEBHOOK_URL for a custom hook. Without these, messages log to the server console; signup/resend may return verifyUrl so the production verify gate can still be tested.',
  }
}
