...
export function listQuotes(tenantId: string, status?: string) {
...
export async function createQuote(tenantId: string, dto: { customerRef: string; notes?: string; lines: Array<{ lineNo: number; skuCode?: string; description: string; qty: number; unitPrice: number }> }) {
...
export async function submitQuote(tenantId: string, id: string, sessionCustomerId?: string) {
  const q = await getQuote(tenantId, id)
  if (q.convertedOrderId) return getQuote(tenantId, id)
