  ...
  return (
    <main style={{ maxWidth: 900, margin: '40px auto', padding: '0 20px', color: '#f8fafc' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <Link href="/quotes" style={{ color: '#93c5fd', fontSize: 13 }}>
          ← All quotes
        </Link>
        <Link href="/login" style={{ color: '#93c5fd', fontSize: 13 }}>
          Re-authenticate →
        </Link>
      </div>
            ...
            <p style={{ marginTop: 16, padding: 12, borderRadius: 10, background: '#0f172a', border: '1px solid #334155' }}>
              ...
              {ADMIN_BASE ? (
                <>
                  {' '}
                  <a
                    href={`${ADMIN_BASE}/orders/${encodeURIComponent(q.convertedOrderId)}`}
