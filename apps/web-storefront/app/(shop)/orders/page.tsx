...
const ADMIN_BASE = process.env.NEXT_PUBLIC_WEB_ADMIN_ORIGIN?.replace(/\/$/, '') ?? ''
    ...
    <div style={{ padding: 24, maxWidth: 1000, margin: '0 auto' }}>
      ...
      {unauthorized ? (
        <p style={{ marginTop: 24, color: 'var(--c-danger)' }}>
          ...
          <Link href="/login" style={{ color: 'var(--c-accent)' }}>
            Login →
          </Link>
        </p>
      ) : null}
      ...
      {!loading && data && data.items.length === 0 ? (
        <p style={{ marginTop: 24, color: 'var(--c-text-3)' }}>
          No orders yet.{' '}
          <Link href="/catalog" style={{ color: 'var(--c-accent)' }}>
            Shop catalog →
          </Link>
        </p>
      ) : null}
            ...
            <tbody>
              {data.items.map((o) => (
                <tr key={o.id}>
                  ...
                  <td>
                    <Link href={`/orders/${encodeURIComponent(o.id)}`} style={{ color: 'var(--c-accent)', fontSize: 13 }}>
                      View
                    </Link>
                    {ADMIN_BASE ? (
                      <>
                        {' · '}
                        <a href={`${ADMIN_BASE}/orders/${encodeURIComponent(o.id)}`} style={{ color: 'var(--c-primary)', fontSize: 13 }} target="_blank" rel="noreferrer">
                          Admin
                        </a>
                      </>
                    ) : null}
